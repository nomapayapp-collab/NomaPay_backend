 ...
 * transactions: historial de movimientos, ya preparado para buy / sell / exchange / transfer / deposit, con moneda de origen, moneda de destino, tasa de cambio aplicada y fee.
...
psql $DATABASE_URL -f db/migrations/0005_add_theme.sql

psql $DATABASE_URL -f db/migrations/0006_add_deleted_at.sql

psql $DATABASE_URL -f db/migrations/0007_add_transaction_message.sql
...
| POST | `/api/wallets/me/exchange` | Sí | Compra/venta/intercambio entre dos monedas, con tasa real y comisión. Envía email de confirmación |
...
El modelo de datos y la infraestructura de auth ya estaban preparados desde el diseño inicial para soportar todo esto sin cambios estructurales grandes — y así fue: ninguna de estas features requirió tocar el esquema principal.
